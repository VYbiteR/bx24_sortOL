...
- [Установка](#-установка)
  - [Установщик для Windows](#установщик-для-windows)
...
- [Подключение своего домена](#-подключение-своего-домена)
  - [Через настройки расширения](#через-настройки-расширения)
  - [В Desktop-клиенте](#в-desktop-клиенте)
- [Интеграция с порталом Битрикс24](#-интеграция-с-порталом-битрикс24)
- [Как пользоваться](#-как-пользоваться)
  - [Папки чатов](#папки-чатов)
...
Для быстрой установки и автоматических обновлений используется `ANIT_bx24_sortOL_Setup.exe`.

- Установщик сам копирует расширение в выбранную папку.
- Обновляет найденный ярлык Битрикс24.
- Регистрирует ежедневную задачу `ANIT bx24_sortOL Updater`. Для проверки и установки обновлений

1. В разделе **Release** нажмите на **Последний релиз** → **Нажмите на ANIT_bx24_sortOL_Setup.exe** и запустите от имени Администратора.
...
1. Перейдите на страницу (в самом приложении): `https://<ваш-домен>/add/ext_bx24sortOL` (можно отправить эту ссылку в чат и перейти по ней).
...
1. Установите приложение: [https://www.bitrix24.ru/apps/app/anit.chat_sorter/](https://www.bitrix24.ru/apps/app/anit.chat_sorter/)
...
Папки помогают вручную группировать чаты во **внутренних чатах** и в **Открытых линиях**. Для этих режимов используются независимые наборы папок: папки Открытых линий не смешиваются с папками внутренних чатов.

#### Как создать папку

1. Откройте панель фильтров расширения.
2. В разделе **Папки** нажмите **Настроить папки**.
3. В открывшемся окне введите название папки. При необходимости выберите смайлик перед названием.
4. Нажмите **Создать**.


![Окно создания папки](gitInf/img/create-folders.jpg)

#### Как добавить чаты в папку

1. В списке чатов зажмите **ПКМ** на одном из чатов, чтобы включить режим множественного выбора.
2. Отметьте нужные чаты.
3. В верхней панели действий нажмите **В папку**.
4. Выберите одну или несколько папок, в которые нужно добавить выбранные чаты.
5. Подтвердите действие. Один чат может одновременно находиться в нескольких папках.


![Выбор чатов и папок](gitInf/img/folders-assign.jpg)

После распределения папки появляются отдельной строкой над списком чатов. Если в папке есть непрочитанные сообщения, на её элементе отображается индикатор.
